...
class branch{
    ...
    static async productList(dbName,branchId){
        return new Promise(async (resolve,reject)=>{
            try{
                let pData=await Execute.Query(dbName,"select sbp.product_id,sbp.supplier_branch_id,sbp.category_id,sbp.sub_category_id,sbp.detailed_sub_category_id,sbp.original_product_id from supplier_branch_product sbp join product p on p.id=sbp.product_id where supplier_branch_id=?",[branchId]);
                resolve(pData);
            }
            catch(Err){
                reject(Err)
    ...
    static async deleteIfDupExistInBranch(dbName,products,branchId){
        ...
        try {
            ...
            let params = [branchId, i.product_id, i.product_id, 0]
            let result = await Execute.Query(dbName, sql, params)
            if (result && result.length>0) {
                ...
                let params1 = [1, branchId, i.product_id, i.product_id]
                let response = await Execute.Query(dbName, sql1, params1)
            } 
            if(index == products.length-1){
                resolve()
            }
        ...
        return new Promise(async(resolve,reject)=>{
            try{
                ...
                for (const [index, i] of products.entries()) {
                        ...
                        where supplier_branch_id=${mainBranchId} and category_id=${i.category_id};`;
