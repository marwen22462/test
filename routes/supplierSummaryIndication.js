   ...
   /* for(j=0 ;j<7;i-- && j++)
    {
        (function(j)
        {
            var sql = "delete from supplier_timings where supplier_id = ? and week_id = ?";
            multiConnection[dbName].query(sql,[supplierId,timingsJSON[i].week_id],function (errr,response) {
               //console.log(".......",errr,response);
                if(errr){
                    console.log("error in updating timings",errr);
                    callback(errr);
                    ...
                    multiConnection[dbName].query(sql,[supplierId,timingsJSON[i].week_id,timingsJSON[i].start_time,timingsJSON[i].end_time,timingsJSON[i].is_open],function(err,result)
                    {
                        if(err){
                            ...
                            if(day=timingsJSON[i].week_id){
                                var sql1='update supplier set status = ? where id =?';
                                multiConnection[dbName].query(sql1,[timingsJSON[i].is_open,supplierId],function (errr,response) {
                                    if(errr){
                                        console.log("error in updating timings",errr);
                                    }
                                    else {
           ...
           /!* if(timingsJSON[i].id == 0)
            {
                ...
                multiConnection[dbName].query(sql,[timingsJSON[i].supplier_id,timingsJSON[i].week_id,timingsJSON[i].start_time,timingsJSON[i].end_time,timingsJSON[i].is_open],function(err,result)
                {
                    ...
                    if(day=timingsJSON[i].week_id){
                        var sql1='update supplier set status = ? where id =?'
                        multiConnection[dbName].query(sql1,[timingsJSON[i].is_open,timingsJSON[i].supplier_id],function (errr,response) {
                            console.log("err in update supplier",errr)
                        })
                    }
                    if(i == jsonLength - 1)
                    {
