    ...
    async.waterfall([
           ...
           async function (cb) {
                ...
                agentList(agentConnection,res, limit, offset,supplierId, 
                    startDate,endDate,order_by,is_desc,is_admin,search,is_download, cb);
            },
            function(list,cb){
                cb(null,list)
                // final.agents = list
...
async function agentList(agentConnection,res, limit, offset,supplierId,startDate,endDate,order_by,is_desc,is_admin,search,is_download, cb){
    ...
    var sql = "select cu.name,cu.supplier_id,cu.supplier_name,cu.email,cu.id as user_id,count(cuo.user_id) as total_orders,IFNULL(sum(commission_ammount),0) as revenue,IFNULL(sum(net_amount),0) as totalNetAmount  "
    sql += " from cbl_user cu left join cbl_user_orders cuo on cuo.user_id = cu.id and cuo.status=5 and DATE(cuo.created_on) >='"+startDate+"' "
