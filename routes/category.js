        ...
        function (name1, cb) {
            ...
            updateCategory(req.dbName,
                ...
                imageName,age_limit, iconName,agent_list,start_time,
                 ...
                 payment_after_confirmation, menu_type,is_dine,commission);
        },
        function (cb) {
...
async function savecategory(dbName,order_instructions,cart_image_upload,res,
    ...
    try{
    var sql = "insert into categories(`commission`,`order_instructions`,`cart_image_upload`,`name`, `supplier_placement_level`,  `image`, `icon`,`created_by`,`is_variant`,`age_limit`,`category_flow`,`agent_list`,`start_time`,`end_time`,`tax`,`is_agent`,`type`,`terminology`,`payment_after_confirmation`,`menu_type`,`is_dine`) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    let reply1=await ExecuteQ.Query(dbName,sql,[commission,order_instructions,cart_image_upload,name, supplierPlaceMentLevel, imageName, iconName, adminId,is_variant,age_limit,category_flow,agent_list,start_time,end_time,tax,is_agent,type,terminology,payment_after_confirmation,menu_type,is_dine])
...
async function updateCategory(dbName,order_instructions,cart_image_upload,res, callback,
     categoryId, name, imageName,age_limit, iconName,agent_list,start_time,end_time,
      tax,is_agent,type,terminology,payment_after_confirmation,menu_type,is_dine,commission) {
    try{
        console.log("=====age_limit==55==" , age_limit);

    var sql = "update categories set commission="+commission+",order_instructions=?,cart_image_upload=?,name = ?,age_limit="+age_limit+",image = ?,icon = ?,agent_list=?,start_time=?,end_time=?,tax=?,is_agent=?,type=?,terminology=?,payment_after_confirmation=?,menu_type=?,is_dine=? where id = ? ";
