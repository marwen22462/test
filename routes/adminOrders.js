...
exports.orderDescriptionV2 = function (req, res) {
    ...
    async.auto({
        ...
        orderdescription: ['blankField', 'authenticate', 'checkauthority', function (cb) {
            ...
            orderFunction.orderDescriptionV2(req.dbName,res, orderId,async function (err, result) {
                if (err) {
                    sendResponse.somethingWentWrongError(res);
                }
                else {
...
exports.confirmPendingOrder = async function (req, res) {
    ...
    async.auto({
            ...
            orderInformation:['calculateDeliveryChargeAccToVehicleType',async function(callback){
                if(enable_base_delivery_charge_on_vehicle_cat && enable_base_delivery_charge_on_vehicle_cat.length>0){                    logger.debug("========$$$$$$$$$=%%%%%%%%%%%%%1111111111111%%%%%==============")
                   ...
                   let data=await Execute.Query(req.dbName,select_query,[orderId]);
            ...
            deliveryAddress:["orderInformation",async function(callback){
                ...
                if(enable_base_delivery_charge_on_vehicle_cat && enable_base_delivery_charge_on_vehicle_cat.length>0){ 
                    ...
                    " usr.reference_address,IFNULL(usr.name,'') as name,IFNULL(usr.phone_number,'') AS phone_number ,usr.directions_for_delivery,usr.address_link,usr.customer_address from orders ors left join user_address usr on usr.id=ors.user_delivery_address where ors.id IN(?)"
                    let deliveryData=await Execute.Query(req.dbName,select_query,[orderId]);
            ...
            orderItemData:['deliveryAddress',async function(callback){
                ...
                if(enable_base_delivery_charge_on_vehicle_cat && enable_base_delivery_charge_on_vehicle_cat.length>0){                    let  select_query="select spb.name as supplier_branch_name,spb.address as supplier_branch_address,spb.latitude,spb.longitude,op.handling_admin,op.id as order_price_id,op.order_id,op.quantity,op.price,op.product_id as item_id,op.product_name as item_name, "+
...
deliveredOrder = async function (req, res) {
    ...
    notificationData: ['payToAgentAndSupplier', function (cb) {
        adminOrders.getValue(req.dbName,res, orderId, function (err, values) {
            ...
            if (err) {
                ...
                net_amount= values.net_amount;
                created_on= moment(values.created_on).format('DD/MM/YYYY HH:mm');
                schedule_date= moment(values.schedule_date).format('DD/MM/YYYY HH:mm');
                if(values.payment_type==0){
                    payment_type = "cash";
                }
                else {
