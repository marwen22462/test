            ...
            function (cb) {
                ...
                products.updateProduct(req.dbName,res, productId, names[0], priceUnit, descriptions[0], sku, barCode, commission,commissionType, 
                    ...
                    payment_after_confirmation,cart_image_upload,
                     ...
                     is_allergy_product,is_appointment,special_instructions,0,0,calories,cb);
    ...
    // let cart_image_upload=req.body.cart_image_upload || 0
                insertProductBySupplierBranch(req.dbName,is_prescribed,res, branchId, categoryId, subCategoryId, detailedSubCategoryId, names[0], priceUnit, descriptions[0], sku, barCode,
                     commission, commissionType, commissionPackage, measuringUnit[0],pricing_type,
                      ...
                      is_subscription_required,allergy_description,
                      is_allergy_product,is_non_veg,is_appointment,special_instructions,calories,cb);
    ...
    async.waterfall([
            ...
            function(product,cb){
                if(product && product.length){
                    ...
                    for(var i =0;i < len;i++){
                        (async function(i){
                            var sql = "SELECT p.tax_type,p.tax_value,p.user_type_id,p.id,p.pricing_type,p.start_date,p.end_date,p.price,p.display_price,p.handling,p.handling_supplier";
                                sql += " ,p.price_type,";
...
exports.addPricingOfProductByAdmin = function (req, res) {
    ...
    if(consts.LOCATION_FLOW.flow==0){
        location_area_wise(res,req.dbName,accessToken, sectionId, productId, startDate, endDate, price, displayPrice, handlingFeeAdmin,
            ...
            offerType, type, id, pricing_type,manValues,adminId,actualPrice,tax_type,tax_value, function (err, data) {
                if (err) {
                    sendResponse.somethingWentWrongError(res);
                } else {
                    var data = []
        ...
        location_lat_long_wise(service_type,
            ...
            tax_type,tax_value,function(err,data){
                if(err){
...
var location_area_wise = function(
 ...
 handlingFeeSupplier,isUrgent,urgentPrice,urgentType,deliveryCharges,minOrder,chargesBelowMinOrder,areaId,
 offerType,type,id,pricing_type,manValues,adminId,actualPrice,tax_type,tax_value,callback   
){
    async.waterfall([
        function (cb) {
            func.checkBlank(res, manValues, cb);
        ...
        function (cb) {
            if (req.body.discountPrice) {
                ...
                if (req.body.houseCleaningPrice) {
                    var houseCleaningPrice = req.body.houseCleaningPrice;
                    insertPricing(dbName,res, productId, startDate, endDate, price, handlingFeeAdmin, handlingFeeSupplier, isUrgent, urgentPrice, deliveryCharges, minOrder, chargesBelowMinOrder, areaId, urgentType, offerType, type, id, displayPrice, houseCleaningPrice, 0, discountPrice, discountStartDate, discountEndDate,pricing_type, tax_type,tax_value,cb);
                }
                else if (req.body.beautySaloonPrice) {
                    var beautySaloonPrice = req.body.beautySaloonPrice;
                    insertPricing(dbName,res, productId, startDate, endDate, price, handlingFeeAdmin, handlingFeeSupplier, isUrgent, urgentPrice, deliveryCharges, minOrder, chargesBelowMinOrder, areaId, urgentType, offerType, type, id, displayPrice, 0, beautySaloonPrice, discountPrice, discountStartDate, discountEndDate,pricing_type,tax_type,tax_value, cb);
                }
                else {
                    insertPricing(dbName,res, productId, startDate, endDate, price, handlingFeeAdmin, handlingFeeSupplier, isUrgent, urgentPrice, deliveryCharges, minOrder, chargesBelowMinOrder, areaId, urgentType, offerType, type, id, displayPrice, 0, 0, discountPrice, discountStartDate, discountEndDate,pricing_type,tax_type,tax_value, cb);
                }
                ...
                if (req.body.houseCleaningPrice) {
                    var houseCleaningPrice = req.body.houseCleaningPrice;
                    insertPricing(dbName,res, productId, startDate, endDate, price, handlingFeeAdmin, handlingFeeSupplier, isUrgent, urgentPrice, deliveryCharges, minOrder, chargesBelowMinOrder, areaId, urgentType, offerType, type, id, displayPrice, houseCleaningPrice, 0, 0, 0, 0,pricing_type,tax_type,tax_value, cb);
                }
                else if (req.body.beautySaloonPrice) {
                    var beautySaloonPrice = req.body.beautySaloonPrice;
                    insertPricing(dbName,res, productId, startDate, endDate, price, handlingFeeAdmin, handlingFeeSupplier, isUrgent, urgentPrice, deliveryCharges, minOrder, chargesBelowMinOrder, areaId, urgentType, offerType, type, id, displayPrice, 0, beautySaloonPrice, 0, 0, 0,pricing_type,tax_type,tax_value, cb);
                }
                else {
                    insertPricing(dbName,res, productId, startDate, endDate, price, handlingFeeAdmin, handlingFeeSupplier, isUrgent, urgentPrice, deliveryCharges, minOrder, chargesBelowMinOrder, areaId, urgentType, offerType, type, id, displayPrice, 0, 0, 0, 0, 0,pricing_type, tax_type,tax_value,cb);
                }
            }
        },
    ], function (error, result) {
...
accessToken,sectionId,productId,startDate,endDate,price,displayPrice,handlingFeeAdmin,
handlingFeeSupplier,isUrgent,urgentPrice,urgentType,deliveryCharges,minOrder,chargesBelowMinOrder,
offerType,type,id,pricing_type,manValues,adminId,actualPrice,discountPrice,discountStartDate,discountEndDate,houseCleaningPrice,beautySaloonPrice,tax_type,tax_value,callback
){
    async.waterfall([
        function (cb) {
            func.checkBlank(res, manValues, cb);
                ...
                if(checkUserType && checkUserType.length>0){
                    ...
                    insertProductPrice(dbName,res, productId, startDate, endDate, price, handlingFeeAdmin,
                        handlingFeeSupplier, isUrgent, urgentPrice, deliveryCharges, minOrder,
                         ...
                         beautySaloonPrice,tax_type,tax_value, cb);
                }
            }
            else{
                insertProductPrice(dbName,res, productId, startDate, endDate, price, handlingFeeAdmin,
                ...
                if(req.service_type==1 || req.service_type==2){
                    ...
                    cb(null);
                        ...
                        if (req.body.houseCleaningPrice) {
                            ...
                            editPricing(req.dbName,res, productPricingId, startDate, endDate, price, handlingFeeAdmin, handlingFeeSupplier, isUrgent, urgentPrice, deliveryCharges, minOrder, chargesBelowMinOrder, areaId, urgentType, offerType, type, id, displayPrice, 0, 0,productId,cb,tax_type,tax_value);
                        }
                    }
    ...
    if (discountPrice == 0) {
        ...
        let result=await ExecuteQ.Query(dbName,sql,[productId, 0, 0])
                ...
                if(result.length){
                    ...
                    var sql1 = "insert into product_pricing(product_id,start_date,end_date,price,handling,handling_supplier,can_urgent,urgent_price,delivery_charges,urgent_type,price_type,house_cleaning_price,beauty_saloon_price,display_price,urgent_value,pricing_type,tax_type,tax_value) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
                    await ExecuteQ.Query(dbName,sql1,[productId, moment(startDate).format('YYYY-MM-DD'), moment(endDate).format('YYYY-MM-DD'), price, handlingFeeAdmin, handlingFeeSupplier, isUrgent, urgentPrice, deliveryCharges, urgentType, offerType, houseCleaningPrice, beautySaloonPrice, displayPrice,urgentPrice,pricing_type,tax_type,tax_value])
...
exports.updateProduct=async function(dbName,res, productId, names, priceUnit, 
    ...
    is_subscription_required,allergy_description,
    is_allergy_product,is_appointment,special_instructions,updateRequestId,by_supplier,calories, callback) {
   try{
...
async function listSupplierBranchProductDetails(dbName,res, branchId, categoryId, subCategoryId, detailedSubCategoryId,limit,offset,serachType,serachText, callback) {
   try{
    ...
    if(serachType==0){
        ...
        var sql = "select "+tblSettingSql+"p.quantity,IF((select count(*) from product where product.parent_id=p.id)>0,1,0) as is_variant,p.pricing_type,(p.quantity-p.purchased_quantity) as left_quantity,p.purchased_quantity,br.id as brand_id,br.name as brand_name,p.special_instructions,p.calories,br.image as brand_image,p.name,p.id,p.is_product,p.duration,p.bar_code,p.product_desc,p.sku,p.is_live,p.making_price,p.commission,p.commission_type,p.commission_package,p.category_id,p.sub_category_id,p.detailed_sub_category_id,c.chemical_tools_price_applicable,c.services_at_home_price_applicable,c.name category_name,c.is_barcode,curr.currency_name from supplier_branch_product sp ";
        sql += " join product p on sp.product_id = p.id left join brands br on br.id=p.brand_id join categories c on c.id = p.category_id join currency_conversion curr ";
...
async function listSupplierBranchProductDetailsV1(dbName,res, branchId, categoryId, subCategoryId, detailedSubCategoryId,limit,offset,serachType,serachText,tags, callback) {
    ...
    var id =[];
     ...
     if(serachType==0){
        ...
        var sql = "select sp.recipe_pdf,p.Size_chart_url,p.special_instructions,p.is_supplier_product_approved,p.is_appointment,p.is_non_veg,p.purchase_limit,p.allergy_description,p.is_allergy_product,p.is_subscription_required,p.calories,p.country_of_origin,p.payment_after_confirmation,p.item_unavailable,p.cart_image_upload,IF((select count(*) from product where product.parent_id=p.id)>0,1,0) as is_variant,(p.quantity-p.purchased_quantity) as left_quantity,c.type,p.interval_flag,p.interval_value, p.making_price,p.product_tags,p.is_prescribed,p.quantity,p.purchased_quantity,br.id as brand_id,br.name as brand_name,br.image as brand_image,p.name,p.id,p.pricing_type,p.is_product,p.duration,p.bar_code,p.product_desc,p.sku,p.is_live,p.commission,p.commission_type,p.commission_package,p.category_id,p.sub_category_id,p.detailed_sub_category_id,c.chemical_tools_price_applicable,c.services_at_home_price_applicable,c.name category_name,c.is_barcode,curr.currency_name,sp.order_no from supplier_branch_product sp ";
         sql += " join product p on sp.product_id = p.id left join brands br on br.id=p.brand_id and br.deleted_by= 0 join categories c on c.id = p.category_id left join currency_conversion curr ";
         sql += " on curr.id = p.price_unit where p.parent_id=? and sp.supplier_branch_id = ? and sp.is_deleted = ?   and sp.category_id = ? and sp.sub_category_id = ? and sp.detailed_sub_category_id = ? " +
             "and (p.id LIKE '%"+serachText+"%' or p.bar_code LIKE '%"+serachText+"%' or p.name LIKE '%"+serachText+"%'" +
...
async function insertProductBySupplierBranch(dbName,is_prescribed,res, branchId, categoryId, subCategoryId, detailedSubCategoryId, name, priceUnit, description, sku, barCode, commission, commissionType, commissionPackage, measuringUnit,
    ...
    is_non_veg,is_appointment,special_instructions,calories,callback) {
    ...
    if(api_version==1){
        ...
        params=[country_of_origin,Size_chart_url,
            ...
            sku, detailedSubCategoryId, 0, branchId, 2, commission,
             ...
             is_subscription_required,allergy_description,
             is_allergy_product,is_non_veg,is_appointment,special_instructions,calories]
    }
    let result=await ExecuteQ.Query(dbName,sql,params);
    console.log("--Exit--")
    callback(null, result.insertId);
...
async function deleteSupplierProductOfBranch(dbName,res, productId, branchId, callback) {
  try{
    ...
    //     }
    //     else {
            var sql3 = "delete sbp from supplier_branch_area_product sbp join product p on sbp.product_id=p.id where  sbp.supplier_branch_id = ? and p.parent_id IN ("+productId+")";
