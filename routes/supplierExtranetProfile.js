    ...
    async.auto({
        ...
        updateEmail:['getEmail',async function (cb) {
            ...
            if(flag==1){
                ...
                // multiConnection[req.dbName].query(sql,[supplierEmail,supplierEmail,supplierId],function (err,result) {
                //     if (err) {
        ...
        updateData:['updateEmail',async function (cb) {
            try{
                var sql = "update supplier set distance_value=?,base_delivery_charges=?,is_dine_in=?,address=?,phone = ? ,fax = ? ,mobile_number_1 = ?,mobile_number_2 = ? ,trade_license_no = ?,business_start_date = ?,name = ?,delivery_radius=?,radius_price=?,license_number=? where id = ? limit 1";
                 await ExecuteQ.Query(req.dbName,sql,[distance_value,base_delivery_charges, is_dine_in,addressEn,telephone,fax,primaryMobile,secondaryMobile,tradeLicenseNo,businessStartDate,supplierNameEn,delivery_radius,radius_price,licenseNumber,supplierId])
                 cb(null);
                }
            catch(Err){
                logger.debug("===Err==",Err);
            ...
            // var sql = "update supplier set address=?,phone = ? ,fax = ? ,mobile_number_1 = ?,mobile_number_2 = ? ,trade_license_no = ?,business_start_date = ?,name = ?,delivery_radius=?,radius_price=? where id = ? limit 1";
           
            // multiConnection[req.dbName].query(sql, [addressEn,telephone,fax,primaryMobile,secondaryMobile,tradeLicenseNo,businessStartDate,supplierNameEn,delivery_radius,radius_price,supplierId], function (err, result) {
            //     if (err) {
            //         console.log("err.....",err);
            //         sendResponse.somethingWentWrongError(res);
            //     }
        ...
        updateSupplierEnglishName:['updateData2',async function(cb){
            ...
            // multiConnection[req.dbName].query(sql,[supplierNameEn,addressEn,supplierId,14],function(err,result)
            // {
        ...
        checkSupplierHeadBranch:['updatePaymentMethods',async function(cb){
                ...
                //             head_branch = 1;
                //             supplierBranchId = result[0].id;
                //             console.log("=====supplierBranchId=========",result[0].id)
                //         }
                //         cb(null)
                //     }
                // })
