...
const listSuppliers = async (req,res)=>{
    ...
    try{
        ...
        let getRegisterSupplier = await  getRegisterSuppliers(req.dbName,res,limit,
            ...
            is_stripe_connected,sequence_wise);
...
async function getRegisterSuppliers(dbName,res,limit,
    ...
    return new Promise(async(resolve,reject)=>{
        try{
            ...
            let query = "select (SELECT message_id FROM `chats` WHERE (`send_to_type`='ADMIN' or `send_by_type`='ADMIN') and (send_by=s.user_created_id or send_to=s.user_created_id) and (`send_to_type`='SUPPLIER' or `send_by_type`='SUPPLIER') order by c_id desc limit 1) as message_id,s.user_created_id, sb.id as default_branch_id,s.is_sponser as is_multibranch, s.country_code,s.gst_price,s.iso,s.self_pickup,s.name,s.id,s.email,s.description,sml.address,"+
...
function clubDataForRegSupplier(res,result) {
    return new Promise((resolve,reject)=>{
        ...
        logger.debug("=======suppliers=>>==",suppliers.length,supplierLength,categories.length)
        var x = 0;
        var y = 0;
        var z = 0;
        var exception = {};
