...
const getMainCategoryList = async (req,res)=>{
    try{
        ...
        if(req.supplier && req.supplier.supplier_id){
            ...
            sql = "select IF((select count(*)  from questions  where questions.category_id=c.id) > 0, 1, 0) as is_question,IF((select count(*)  from product  where  product.category_id=c.id and product.is_deleted='0') > 0, 1, 0) as is_product,IF((select COUNT(*) from categories cts where cts.parent_id=c.id )>0,1,0) as is_sub_category,c.payment_after_confirmation,c.terminology,c.category_flow,c.is_agent,c.agent_list,c.order_instructions,c.is_quantity,c.type,c.is_variant,c.commission,c.age_limit,c.id,c.image,c.icon,c.illustration,c.is_live,c.start_time,c.end_time,c.tax,c.is_dine, c.menu_type  from categories c join supplier_category sc on sc.category_id =c.id where c.parent_id=? and c.is_deleted= ? and c.is_live=1 and sc.supplier_id =? and c.name LIKE '%" + search + "%' group by c.name  LIMIT ?,?"
            if(params.limit==0){
            ...
            // and product.sub_category_id=c.id and product.detailed_sub_category_id=c.id
            sql = "select IF((select count(*)  from questions  where questions.category_id=c.id) > 0, 1, 0) as is_question,IF((select count(*)  from product  where  product.category_id=c.id ) > 0, 1, 0) as is_product,IF((select COUNT(*) from categories cts where cts.parent_id=c.id )>0,1,0) as is_sub_category,c.terminology,c.order_instructions,c.category_flow,c.payment_after_confirmation,c.is_agent,c.agent_list,c.is_quantity,c.type,c.is_variant,c.commission,c.age_limit,c.id,c.image,c.icon,c.illustration,c.is_live,c.start_time,c.end_time,c.tax, c.is_dine, c.menu_type from categories c where c.parent_id=?  and c.name LIKE '%" + search + "%'  LIMIT ?,?"
            if(params.limit==0){
                sql = sql.replace("LIMIT ?,?","")
                condition = [0]
            }else{
                condition = [0,params.offset,params.limit]    
            }
                count_condition = [0]        
        }
