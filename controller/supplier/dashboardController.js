...
function TotalOrderRevenue(dbName, start_date, end_date, supplierId,supplierBranchId=0) {
    return new Promise(async (resolve, reject) => {
        ...
        let result1=await ExecuteQ.Query(dbName,q,[supplierId]);
        // multiConnection[dbName].query(q, [supplierId], function (err, result1) {
            var sql = "SELECT IFNULL(DAYOFWEEK(`created_on`),0) as week_day,IFNULL(DATE(`created_on`),0) as created_at,IFNULL(SUM(net_amount),0) as total_revenue from orders where DATE(created_on) >=? and DATE(created_on) <=? and supplier_branch_id = ? and status = 5 group by DATE(created_on)"
            if (result1.length) {
                for (var i = 0; i < result1.length; i++) {
                    (async function (i) {
                        try{
