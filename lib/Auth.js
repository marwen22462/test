...
const getLogoUrl=(dbName)=>{
    ...
    return new Promise(async (resolve,reject)=>{
        try {
          ...
          let data=  await ExecuteQ.Query(dbName,"select `key`,`value` from tbl_setting where `key`=?  or `key`=?",["logo_url","logo_url_v2"]);
...
exports.checkCblAuthority = async (req,res,next)=>{   
    ...
    //     else {
           ...
           // let settingLogoData=await ExecuteQ.Query(DbName,"select `key`,`value` from tbl_setting where `key`=?  or `key`=?",["logo_url","logo_url_v2"]);
            ...
            console.log(settingLogoData,"loglogolgologolgologologolgogolgog")

            let logoUrl=settingLogoData && settingLogoData.length>0?settingLogoData.logo_url:"";
            let logoUrl_v2=settingLogoData && settingLogoData.length>0?settingLogoData.logo_url_v2:"";
